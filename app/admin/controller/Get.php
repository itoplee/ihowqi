...
class Get extends Http
{
    ...
    {    	
        ...
        if($count > 0){
            ...
            for($i=0; $i<$count; $i++){
                $id = $data['list'][$i]['id'];
                $arr[$i] = $data['list'][$i];
                $arr[$i]['finishCount'] = $Event2->getStateCount($id, 2) + $Event3->getStateCount($id, 2);
            }
            $data['list'] = $arr;
        }
        return new HttpResult($data);
    ...
    public function getProjectProcess() {
        ...
        if($count > 0){                    
            foreach($companys as $company) {
                ...
                $diffArr = $this->getUseTimeDiff($this->getEvent('ProjectCompanyExecute')->getUseTimeDiff($id, $company['company_id']));
        ...
        $arr[$index]['partIn'] = $arr[$index]['unfinish'] + $arr[$index]['compete'] + $arr[$index]['refuse'] + $arr[$index]['full'];
        $diffArr = $this->getUseTimeDiff($this->getEvent('AccountProject')->getUseTimeDiff($id));
        $arr[$index]['min'] = $diffArr['min'];
        $arr[$index]['max'] = $diffArr['max'];
        $arr[$index]['avg'] = $diffArr['avg'];
        $arr[$index]['sum'] = $diffArr['sum'];
    ...
    private function getUseTimeDiff($arr) 
    {
        $data = ['min' => 0, 'max' => 0, 'avg' => 0, 'sum' => 0, 'total' => 0];
        $count = count($arr);
        $min = 0;
        $max = 0;
        $sum = 0;
